...
Breakfast is at the hotel each morning after the first night; August 14 is the one home-start exception. There is no brunch: each day has lunch and dinner only, with exactly one substantial restaurant dine and the other meal intentionally light. A compact “Simple food + extra time” switch offers one named quick meal and the best nearby use of the saved time. A separate collapsed “Along-the-way options” panel provides route-side attractions with visit length, route impact, a strict go/no-go gate, the closest named parking target, and official information. Choose no more than one; on the longest driving days, it replaces a break and is allowed only when the live hotel ETA remains protected.

All hotels for the trip are already booked, so the return day suggests no extra overnight. Instead, plan a two-driver rotation with generous rest and driver-swap breaks and make an honest go/rest decision at westbound Mallorytown North based on fatigue, weather, traffic, and the child's condition. If either driver is unfit to continue, stop and rest until safe rather than driving to the clock.
